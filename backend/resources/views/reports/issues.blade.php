...
<head>
    ...
    <style>
        ...
        .badge-unresolved { background-color: #fee2e2; color: #991b1b; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="report-info">
        ...
        <div><strong>Total Data:</strong> {{ $issues->count() }} Kendala</div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $issue->created_at ? $issue->created_at->format('d/m/Y H:i') : '-' }}</td>
                <td>{{ $issue->report && $issue->report->schedule && $issue->report->schedule->user ? $issue->report->schedule->user->name : '-' }}</td>
                <td>{{ $issue->report && $issue->report->schedule && $issue->report->schedule->user ? ucfirst($issue->report->schedule->user->role) : '-' }}</td>
                <td>{{ $issue->report && $issue->report->schedule && $issue->report->schedule->checkpoint ? $issue->report->schedule->checkpoint->name : '-' }}</td>
                <td>{{ $issue->issue_description }}</td>
                <td><span class="badge badge-{{ $issue->is_resolved ? 'resolved' : 'unresolved' }}">{{ $issue->is_resolved ? 'Terselesaikan' : 'Belum Terselesaikan' }}</span></td>
            </tr>
            @endforeach
        </tbody>
    </table>
